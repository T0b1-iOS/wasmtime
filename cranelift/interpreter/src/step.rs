    ...
    Ok(match inst.opcode() {
        ...
        Opcode::Umullohi | Opcode::Smullohi => {
            ...
            let (arg0, arg1) = if inst.opcode() == Opcode::Umullohi {
                let arg0 = arg(0)?.convert(ValueConversionKind::ToSigned)?.convert(ValueConversionKind::ZeroExtend(double_length))?;
                let arg1 = arg(1)?.convert(ValueConversionKind::ToSigned)?.convert(ValueConversionKind::ZeroExtend(double_length))?;
                (arg0, arg1)
            } else {
                let arg0 = arg(0)?.convert(ValueConversionKind::ToSigned)?.convert(ValueConversionKind::SignExtend(double_length))?;
                let arg1 = arg(1)?.convert(ValueConversionKind::ToSigned)?.convert(ValueConversionKind::SignExtend(double_length))?;
                (arg0, arg1)
            };
